...
class TestFormatRefreshTree:
    """Tests for _format_refresh_tree function in commands.py (lines 772-832)."""

    def test_format_refresh_tree_empty_models(self):
        """Test _format_refresh_tree with no modified models."""
        from dbt_meta.commands import _format_refresh_tree
        from unittest.mock import MagicMock

        parser = MagicMock()

        result = _format_refresh_tree(
            all_models=[],
            modified_models=[],
            parser=parser,
            models_dict={}
        ...
        assert result == "No modified models found"

    def test_format_refresh_tree_single_model(self):
        """Test _format_refresh_tree with single modified model."""
        from dbt_meta.commands import _format_refresh_tree
        from unittest.mock import MagicMock

        parser = MagicMock()

        modified = [{'unique_id': 'model.project.core__events', '_git_status': 'uncommitted'}]
        all_models = [{'unique_id': 'model.project.core__events'}]
        models_dict = {'model.project.core__events': {'depends_on': {'nodes': []}}}

        result = _format_refresh_tree(all_models, modified, parser, models_dict)

        assert '🔴' in result  # Uncommitted icon
        assert 'core__events' in result

    def test_format_refresh_tree_with_committed_status(self):
        """Test _format_refresh_tree with committed status."""
        from dbt_meta.commands import _format_refresh_tree
        from unittest.mock import MagicMock

        parser = MagicMock()

        modified = [{'unique_id': 'model.project.core__events', '_git_status': 'committed'}]
        all_models = [{'unique_id': 'model.project.core__events'}]
        models_dict = {'model.project.core__events': {'depends_on': {'nodes': []}}}

        result = _format_refresh_tree(all_models, modified, parser, models_dict)

        assert '✅' in result  # Committed icon
        assert 'core__events' in result

    def test_format_refresh_tree_with_unknown_status(self):
        """Test _format_refresh_tree with unknown git status."""
        from dbt_meta.commands import _format_refresh_tree
        from unittest.mock import MagicMock

        parser = MagicMock()

        modified = [{'unique_id': 'model.project.core__events', '_git_status': ''}]
        all_models = [{'unique_id': 'model.project.core__events'}]
        models_dict = {'model.project.core__events': {'depends_on': {'nodes': []}}}

        result = _format_refresh_tree(all_models, modified, parser, models_dict)

        assert '•' in result  # Default bullet
        assert 'core__events' in result

    def test_format_refresh_tree_with_children(self):
        """Test _format_refresh_tree with child models."""
        from dbt_meta.commands import _format_refresh_tree
        from unittest.mock import MagicMock

        parser = MagicMock()

        modified = [{'unique_id': 'model.project.stg_events', '_git_status': 'uncommitted'}]
        all_models = [
            {'unique_id': 'model.project.stg_events'},
            {'unique_id': 'model.project.core_events'},
            {'unique_id': 'model.project.mart_events'},
        ]
        models_dict = {
            'model.project.stg_events': {'depends_on': {'nodes': []}},
            'model.project.core_events': {'depends_on': {'nodes': ['model.project.stg_events']}},
            'model.project.mart_events': {'depends_on': {'nodes': ['model.project.core_events']}},
        }

        result = _format_refresh_tree(all_models, modified, parser, models_dict)

        assert 'stg_events' in result
        assert 'core_events' in result
        assert 'mart_events' in result
        assert '├──' in result or '└──' in result  # Tree structure

    def test_format_refresh_tree_avoids_cycles(self):
        """Test _format_refresh_tree handles cyclic dependencies."""
        from dbt_meta.commands import _format_refresh_tree
        from unittest.mock import MagicMock

        parser = MagicMock()

        modified = [{'unique_id': 'model.project.model_a', '_git_status': 'uncommitted'}]
        all_models = [
            {'unique_id': 'model.project.model_a'},
            {'unique_id': 'model.project.model_b'},
        ]
        # Circular dependency (shouldn't happen but code handles it)
        models_dict = {
            'model.project.model_a': {'depends_on': {'nodes': ['model.project.model_b']}},
            'model.project.model_b': {'depends_on': {'nodes': ['model.project.model_a']}},
        }
    ...
    def test_format_refresh_tree_multiple_modified(self):
        """Test _format_refresh_tree with multiple modified models."""
        from dbt_meta.commands import _format_refresh_tree
        from unittest.mock import MagicMock
        ...
        modified = [
            {'unique_id': 'model.project.stg_users', '_git_status': 'uncommitted'},
            {'unique_id': 'model.project.stg_events', '_git_status': 'committed'},
        ]
        all_models = [
            {'unique_id': 'model.project.stg_users'},
            {'unique_id': 'model.project.stg_events'},
            {'unique_id': 'model.project.core_users'},
        ]
        models_dict = {
            'model.project.stg_users': {'depends_on': {'nodes': []}},
            'model.project.stg_events': {'depends_on': {'nodes': []}},
            'model.project.core_users': {'depends_on': {'nodes': ['model.project.stg_users']}},
        }
