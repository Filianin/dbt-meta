...
**Power BI visual layout (SCHEMA_VERSION 1.1):** `powerbi artifacts` runs a second pass calling Fabric Get Report Definition (`getDefinition?format=PBIR-Legacy`, parsed by `powerbi/pbir_parser.py`) per report, attaching `ReportEntry.pages` = `[{name, visuals:[{type, fields:{role:[{table,column,kind}]}}]}]` (raw model names; canonical role map + raw fallback; `kind`=measure|column). `pages` is **sparse** (omitted when no layout). The pass needs its **own Fabric-scoped token** (`get_fabric_token`, audience `https://api.fabric.microsoft.com/.default`) — the Scanner's Power BI-scoped token 401s on the Fabric data-plane — and the SP must be a *member* (Contributor) of the workspaces. `get_report_definition` isolates every failure (returns `None`), so one bad report never aborts the scan; summary reports `N/M reports got layouts`. `--no-layouts` skips the pass. `PBIR-Legacy` is mandatory: classic reports reject `format=PBIR`.
